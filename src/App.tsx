...
// Recognizes titles produced by either past buggy parser (raw hostname+path,
// or a leftover long hex hash/token from an earlier "grab nearby segments"
// version) so "Re-parse Titles" can safely fix them without touching titles
// the user typed or edited by hand.
function looksAutoGenerated(title: string, url: string): boolean {
  if (!title) return true;
  if (title === oldAutoTitle(url)) return true;
  if (/\b[0-9a-f]{16,}\b/i.test(title)) return true;
  return false;
}
...
export default function App() {
  ...
  async function handleRetitleAll() {
    ...
    const candidates = allLinks
      ...
      .filter(({ l, next }) => next && next !== l.title && looksAutoGenerated(l.title, l.url));
