...
// Doing one `update().eq("id", ...)` round-trip per link sequentially (await
// in a loop) is what made batch actions over dozens/hundreds of links read
// as the app "freezing" — each request waits for the previous one to finish
// before starting. This fires them concurrently instead (chunked so a very
// large batch doesn't open hundreds of sockets at once), which fixes the
// freeze without changing what each request does.
//
// NOTE: an earlier version of this tried a single `upsert(...)` round-trip
// per chunk, since PostgREST can merge many rows in one call. That broke in
// practice: `upsert` is implemented as `INSERT ... ON CONFLICT`, and
// Postgres rejects an explicit value for `id` in the INSERT list whenever
// the column is `GENERATED ALWAYS AS IDENTITY` (as opposed to `GENERATED BY
// DEFAULT`) — even though the row already exists and it's really just an
// update. Plain per-row `update()` calls don't touch `id` as a value at all,
// so they don't hit that restriction.
async function dbUpdateHttpLinksUrlsBulk(updates: { id: string; url: string }[]): Promise<void> {
  ...
  for (let i = 0; i < updates.length; i += CHUNK_SIZE) {
    ...
    const results = await Promise.all(
      chunk.map((u) => supabase.from("http_links").update({ url: u.url.slice(0, 4000) }).eq("id", Number(u.id)))
    );
